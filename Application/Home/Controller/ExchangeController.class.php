...
class ExchangeController extends HomeController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","upTrade","upMytx","autoClearOvertimeOrder","autoEveryDayOnce","overTimeC2CExchangeOrder","overTimeC2CExchangeOrderAll","orderQueren","cancelOrder","autoSellCNCstatus","autoBuyCNCstatus","bank","payPersonalParams","payOrganizationParams","editPayParams","getPayParams","delPayParams","kycOrganization","kycOrganization_Handle","setPayParamsStatus","saveQrcode","upPersonalCode","upControlEdit","successRateList","rushOrder","resetOrderQueen","repostOrderNotify","checkNotice");
		if(!in_array(ACTION_NAME,$allow_action)){
			 Log::record("ExchangeController非法操作！".ACTION_NAME, Log::INFO);
			$this->error(L("非法操作！"));
		}
	}
	
	public function index($order_mode = 0, $c2c_coin = Anchor_CNY, $search_field = null, $search_value = null)
	{
		if(checkstr($order_mode) || checkstr($c2c_coin) || checkstr($search_value)){
			$this->error(L('您输入的信息有误！'));
		}

		if(strpos($search_field, 'order') === false && checkstr($search_field)){
			$this->error(L('您输入的信息有误！'));
		}

		if (!userid()) {
			$this->assign('logins', 0);
			$this->assign('checkNotice', 0);
		} else {
		...
		if(is_string($search_value)){
			$search_value = trim($search_value); //去除字符串两边的空格
		}
		$where = array();
		//添加搜索条件
		if(is_string($search_field)){
			...
			switch ($search_field) {
				...
				case 'return_order_id':
					if(is_string($search_value) && $search_value != ''){
						$where[$search_field] = $search_value;
						$this->assign('search_value', $search_value);
					}
					break;
				case 'manual_order':
					$where['notifyurl'] = ['exp','is null'];
					break;
				case 'auto_order':
			...
			$this->assign('search_field', '');
			$this->assign('search_value', '');
		}
		...
		// 搜索账户信息
		$manualPayParams = M('payparams_list')->where(['userid'=>$userid, 'status'=>1,'check_status'=>1,'is_manual_account'=>1])->field('id,truename')->select();
		if(count($manualPayParams) > 0){
			$this->assign('manualPayParams', $manualPayParams);
		...
		switch ($c2c_coin) {
			case Anchor_CNY:
				$where['type'] = $c2c_coin;
				$this->assign('buy_price', $configs['mycz_uprice']);
				$this->assign('sell_price', $configs['mytx_uprice']);
				break;
			case 'btc':
				$where['type'] = $c2c_coin;
				$cur_price = C('market')['btc_'.Anchor_CNY]['new_price'];
				$this->assign('buy_price', $cur_price);
				$this->assign('sell_price', $cur_price);
				break;
			case 'usdt':
				$where['type'] = $c2c_coin;
				$cur_price = C('market')['usdt_'.Anchor_CNY]['new_price'];
				$this->assign('buy_price', $cur_price);
				$this->assign('sell_price', $cur_price);
				break;
			default:
				$where['type'] = Anchor_CNY;
				$c2c_coin = Anchor_CNY;
				$this->assign('buy_price', $configs['mycz_uprice']);
				$this->assign('sell_price', $configs['mytx_uprice']);
				break;
		}
		$this->assign('c2c_coin', $c2c_coin);

		$cur_time = time();
		//对应的剩余金额
		$UserCoin = M('user_coin')->where(array('userid' => $userid))->find();
		$cny['ky'] = round($UserCoin[$c2c_coin], 2) * 1;
		$cny['ky'] = sprintf("%.2f", $cny['ky']);
		//交易中的金额
		$echange_where = array('userid'=>$userid,'otype'=>2,'type'=>$c2c_coin,'status'=>['lt',3]);
		$echange_where['addtime'] = ['between', [$cur_time-24*60*60, $cur_time]]; 
		$sell_order_amount = D('ExchangeOrder')->where($echange_where)->sum('mum');
		$sell_order_amount = isset($sell_order_amount)?$sell_order_amount:0;
		unset($echange_where['userid']);
		$echange_where['aid'] = $userid;
		$echange_where['otype'] = 1;
		$echange_where['status'] = ['in', '1,2'];
		$buy_order_amount = D('ExchangeOrder')->where($echange_where)->sum('mum');
		$buy_order_amount = isset($buy_order_amount)?$buy_order_amount:0;
		$cny['exchanging'] = $sell_order_amount + $buy_order_amount;
		$cny['exchanging'] = round($cny['exchanging'], 2);
		$cny['exchanging'] = sprintf("%.2f", $cny['exchanging']);
		$this->assign('cny', $cny);
		...
		switch ($order_mode) {
			...
			case '1': //卖出市场展示卖出订单
				...
				break;
			case '2': //我的订单
				$where = $this->getOrderWhereWithMode($userid, $order_mode, $search_field, $search_value, $c2c_coin);
				break;
			default:
				$where['otype']  = 1; //买入订单
				$where['status'] = 0;
		...
		$show = $Page->show();
		$list = $this->getOrderList($where, $Page, $search_field, $search_value);

		//获取交易账户提示
		$tips = $this->getExchangePayparamsTips();
	...
	//根据条件获取订单列表
	private function getOrderList($where, $Page, $search_field, $search_value){
		$list = array();
		//存在具体的搜索条件，就不按照催单排序了
		if(is_string($search_field) && $search_field != '' && is_string($search_value) && $search_value != ''){
			switch ($search_field) {
				case 'orderid':
				case 'out_order_id':
				case 'return_order_id':
					$order_info_list = D('ExchangeOrder')->where($where)->order('id desc')->select();
					if(is_array($order_info_list)){
                        $list = array_merge($list, $order_info_list);
                    }
                    //$order_info = D('ExchangeOrder')->where($where)->order('id desc')->find();
					//if(!empty($order_info)) {array_push($list, $order_info);}
					break;
				default:
			...
			if(is_array($where) && !array_key_exists('rush_status', $where)){
			...
			}elseif(is_string($where) && strpos($where, 'rush_status') === false ){
				//催单的订单
	...
	//获取订单查询条件通过显示方式
	private function getOrderWhereWithMode($userid, $order_mode, $search_field, $search_value, $c2c_coin){
		...
		if(is_string($search_field)){
			...
			switch ($search_field) {
				...
				case 'return_order_id':
					if(is_string($search_value) && $search_value != ''){
						$where = "{$search_field} = '{$search_value}' AND type = '{$c2c_coin}' AND (userid = {$userid} OR aid = {$userid})";
					}else{
						$where = "type = '".$c2c_coin."' AND (userid = {$userid} OR aid = {$userid})";
					}
					break;
				case 'manual_order':
					$where = "notifyurl IS NULL AND type = '{$c2c_coin}' AND (userid = {$userid} OR aid = {$userid})";
					break;
				case 'auto_order':
					$where = "notifyurl IS NOT NULL AND type = '{$c2c_coin}' AND (userid = {$userid} OR aid = {$userid})";
					break;
				case 'auto_buy_order':
					$where = "notifyurl IS NOT NULL AND type = '{$c2c_coin}' AND (userid = {$userid} OR aid = {$userid}) AND otype = 1";
					break;
				case 'auto_comfirm_fail':
					$where = "notifyurl IS NOT NULL AND type = '{$c2c_coin}' AND (userid = {$userid} OR aid = {$userid}) AND rush_status = 2";
					break;
			}
		}else{
			$where = "type = '".$c2c_coin."' AND (userid = {$userid} OR aid = {$userid})";
		...
		switch ($order_mode) {
			case '3'://我的自动确认订单
				$paytype_channelid_list = M('paytype_config')->where(['status'=> 1, 'is_auto_notify'=>1])->field('channelid')->select();
				foreach ($paytype_channelid_list as $key => $value) {
					array_push($channelid_array, $value['channelid']);
				}
				$channelid_str = implode(",", $channelid_array);
				$where = $where . " AND pay_channelid IN ($channelid_str)";
				break;
			case '4'://我的手动确认订单
				$paytype_channelid_list = M('paytype_config')->where(['status'=> 1, 'is_auto_notify'=>0])->field('channelid')->select();
				foreach ($paytype_channelid_list as $key => $value) {
					array_push($channelid_array, $value['channelid']);
				}
				$channelid_str = implode(",", $channelid_array);
	...
	// 买入处理
	public function upTrade($price, $num, $otype, $orderid, $coin_type)
	{
		if (!userid()) {
			$this->error(L('请先登录！'));
		}
		...
		if($userinfo['auto_c2c_sell_status']){
			$this->error(L('开启自动卖出状态下无法进行买入下单和接单'));
		}
		...
		if($orderid){
        	...
        	if($order_info){
        		...
        		$this->error(L('订单状态异常，接单失败orderid='.$orderid));
        	}
		}else{
	...
	// 卖出处理
	public function upMytx($price, $num, $otype, $orderid, $coin_type, $payparams_id)
	{
		if (!userid()) {
			$this->error(L('请先登录！'));
		}
		...
		// 过滤非法字符----------------S
        if (checkstr($price) || checkstr($num) || checkstr($otype) || checkstr($orderid) || checkstr($payparams_id)) {
            $this->error('您输入的信息有误！');
        }
        // 过滤非法字符----------------E
		...
		if($userinfo['auto_c2c_sell_status']){
			$this->error(L('开启自动卖出状态下无法进行手动卖出下单和接单'));
		}

		if(!$payparams_id || $payparams_id == 0){
        	...
        	$this->error(L('没有选择交易用的账户'));
		}else{
			$where = ['id'=>$payparams_id,'userid'=>$userid,'status'=>1,'check_status'=>1,'is_manual_account'=>1];
			$account_info = M('payparams_list')->where($where)->find();
		}
    ...
    private function afterAddExchangeOrder($orderData){
	 ...
	 //清除2小时内没有交易的CNC交易兑换订单
    public function overTimeC2CExchangeOrder()
    {
    	//7天内的时间
        $dayTime = 7*24*60*60;
        $curtime = time();
        $start_time = $curtime-$dayTime;

    	//充值订单每24小时处理一次
    	$map['updatetime'] = array('between',[$start_time, ($curtime - 24*60*60)]);
        $map['status'] = 0;
        $map['otype'] = 1; //买入(买入的未交易订单可以直接取消的原因是没有扣除相应货币，卖出已经扣除故不能直接取消)
        D('ExchangeOrder')->where($map)->save(array('status' => 8,'endtime' => time()));

        //手动提现订单每24小时处理一次
        $map['otype'] = 2; //卖出
        $map['status'] = 0;
        $limit_num = 5; //每次最多处理5个
        $exchangeOrderList = D('ExchangeOrder')->where($map)->limit($limit_num)->order('id asc')->select();
        foreach ($exchangeOrderList as $key => $value) {
        	$this->myChexiaoWithOrderInfo($value);
        }

        //已被接单的订单每20分钟处理一次
        $map['updatetime'] = array('between',[$start_time, ($curtime - 20*60)]);
        $map['otype'] = 1; //买入
        $map['status'] = 1;
        $limit_num = 8; //每次最多处理8个
        $exchangeOrderList = D('ExchangeOrder')->where($map)->limit($limit_num)->order('id asc')->select();
        foreach ($exchangeOrderList as $key => $value) {
        	if(isAutoC2COrder($value)){
        		R("Pay/PayExchange/autoExchangeC2COrderUser", array($value));
        	}else{
        		$this->myChexiaoWithOrderInfo($value);
        	}
        }
        ...
        $map['otype'] = 2; //卖出
        $map['status'] = 1;
        $map['pay_channelid'] = ['gt', 0];
        $map['payparams_id'] = ['gt', 0];
        $limit_num = 5; //每次最多处理5个
        $exchangeOrderList = D('ExchangeOrder')->where($map)->limit($limit_num)->order('id asc')->select();
        foreach ($exchangeOrderList as $key => $value) {
        	$this->myChexiaoWithOrderInfo($value);
        ...
        //处理结束的催单取消请求
        $map['updatetime'] = array('between',[$start_time, ($curtime - 30*60)]);
        unset($map['otype']);
        unset($map['notifyurl']);
        $map['status'] = ['in','3,8']; //完成或取消订单
        $map['rush_status'] = ['gt', 0]; //取消订单
        //撤销催单状态
        D('ExchangeOrder')->where($map)->save(array('rush_status' => 0));

        //限定数量的自动提现订单的超时处理
        $limit_num = 5; //每次最多处理5个
        $res = R("Pay/PayExchange/cancelOverTimeC2COrder", array($limit_num));
        
        return $res;
    }

    //清除今日所有的未成功交易订单
    public function overTimeC2CExchangeOrderAll()
    {
    	//7天内的时间
        $dayTime = 7*24*60*60;
        $curtime = time();
    	...
    	//充值订单每半小时处理一次
    	$map['updatetime'] = array('between',[$start_time, ($curtime - 30*60)]);
        $map['status'] = 1; //已被接单的订单
        $exchangeOrderList = D('ExchangeOrder')->where($map)->select();
        foreach ($exchangeOrderList as $key => $value) {
        	$this->myChexiaoWithOrderInfo($value);
        }

        //所有自动提现订单的超时处理
        $res = R("Pay/PayExchange/cancelOverTimeC2COrder");

        return $res;
    }

    // C2C订单确认成功
	public function orderQueren($orderid, $paypassword, $real_amount)
	{
		if(checkstr($orderid) || checkstr($paypassword) || checkstr($real_amount)){
			$this->error(L('您输入的信息有误！'));
		}
		if (empty($orderid)) {
		...
		if (!empty($real_amount) && isset($order_info['real_amount'])) { //修改过真实金额，才会处理真实金额的逻辑
            $order_info['real_amount'] = $real_amount;
        }
		...
		if ($res === true) {
		    $paymentInfo = M('exchange_payment_record')->where([
        	    'return_order_id'   => $order_info['return_order_id'],
    	        'real_amount'       => $order_info['mum'],
    	        'bankcard'          => $order_info['bankcard'],
    	        'status'            => 0
    	    ])->find();
    	    if ($paymentInfo) {
	...
	public function resetOrderQueen($orderid, $paypassword)
	{
		...
		$order_info = D('ExchangeOrder')->where(array('orderid' => $orderid))->find();
		operation_log($userid, 2, "C2C订单重置成功orderid = ".$orderid);
    ...
    // C2C买入撤销
	private function myczChexiao($orderid, $type = 0)
	{
		...
		$rs = $this->myChexiaoWithOrderInfo($order_info);
		if ($rs === true) {
			if($type == 0){
				$this->success('操作成功！');
			}
	...
	// C2C卖出撤销
	private function mytxChexiao($orderid, $type = 0)
	{
		if (checkstr($orderid) ||checkstr($type)) {
			$this->error(L('您输入的信息有误！'));
		}
	...
	//通过orderinfo撤销订单
	private function myChexiaoWithOrderInfo($order_info){
		if(is_array($order_info)){
			return R("Pay/PayExchange/cancelC2COrderWithOrderInfo", array($order_info));
		}else{
			return false;
		}
	}
	...
	public function autoSellCNCstatus(){
		...
		if($res){
			...
			$this->assign('auto_c2c_sell_status', $userinfo['auto_c2c_sell_status']);
			$this->error('操作失败！');
		}
	}
	...
	public function autoBuyCNCstatus(){
		...
		if($res){
			...
			$this->success('操作成功，优惠订单20分钟内确认，否则将自动取消！');
		}else{
	...
	// 账户管理
	public function bank($is_personal=1)
	{
		if (!userid()) {
			$this->error(L('请先登录！'),U('Login/index'));
		}
		...
		$this->assign('is_personal', $is_personal);
	...
	//更新支付参数
	public function editPayParams($id, $channelid, $truename, $mch_id, $signkey, $appid, $appsecret, $domain_record, $subject, $is_manual_account, $is_defined, $status, $paypassword)
	{
		// 过滤非法字符----------------S
	...
	//获取支付参数
	public function getPayParams($id)
	{
		// 过滤非法字符----------------S
		if (checkstr($id)) {
			$this->error(L('您输入的信息有误！'));
	...
	public function upPersonalCode($qrcode, $mch_id, $channelid, $truename, $paypassword){
		// 过滤非法字符----------------S
		if (checkstr($mch_id)||checkstr($channelid)||checkstr($truename)||checkstr($paypassword)) {
		...
		//Log::record("upPersonalCode getQRcodeToUrl= ".$signkey, Log::INFO);
	...
	//展示账户成功率
	public function successRateList($channelid){
		// 过滤非法字符----------------S
		...
		$where = ['userid'=>$userid, 'channelid'=>$channelid, 'status'=>1, 'check_status'=> 1];
		...
		$list = M('payparams_list')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->field('channelid,truename,is_personal,success_rate_list,max_fail_count')->select();
		if(!empty($list)){
			foreach ($list as $key => $successRate) {
				if($successRate['is_personal']){
					$is_personal = 1;
				}
				if(isset($successRate['success_rate_list'])){
					$success_rate_list = json_decode($successRate['success_rate_list'], true);
					if(!empty($success_rate_list)){
						$list[$key]['success_rate1'] = $success_rate_list[1][2];
						$list[$key]['success_rate2'] = $success_rate_list[2][2];
						$list[$key]['success_rate3'] = $success_rate_list[3][2];
						$list[$key]['success_rate4'] = $success_rate_list[4][2];
						$list[$key]['success_rate5'] = $success_rate_list[5][2];
						$list[$key]['success_rate6'] = $success_rate_list[0][2];
					}else{
						$list[$key]['success_rate1'] = 0;
						$list[$key]['success_rate2'] = 0;
						$list[$key]['success_rate3'] = 0;
						$list[$key]['success_rate4'] = 0;
						$list[$key]['success_rate5'] = 0;
						$list[$key]['success_rate6'] = 0;
					}
				}else{
					$list[$key]['success_rate1'] = 0;
					$list[$key]['success_rate2'] = 0;
	...
	//补发订单通知
	public function repostOrderNotify($orderid){
		if (checkstr($orderid)) {
			$this->error(L('您输入的信息有误！'));
		}
		$order_info = D('ExchangeOrder')->where(['orderid'=>$orderid])->find();
        if($order_info){
            $res = R("Pay/PayExchange/requestMemberOrderStatus", array($order_info, 2));
            if($res){
            	$this->success(L('补发成功！'));
            }else{
            	$this->success(L('补发成功,未返回ok！'));
            }
        }
        $this->error(L('补发失败,订单不存在！'));
	}
	...
	//获取交易账户信息的提示
	private function getExchangePayparamsTips(){
		...
		$where = array(
			'userid' 			=> userid(),
			'low_money_count'	=> ['egt', 3],
		);
		//低收入提示
		$low_payparams = M('payparams_list')->where($where)->find();
		if(!empty($low_payparams)){
			$tips['low_payparams'] = '账户【'.$low_payparams['truename'].'】已经连续3天收入低于预期，请检查是否异常，取消提示状态为关闭一次！';
		}

		//限额提示
		unset($where['low_money_count']);
		$where['limit_amount_status'] = 1;
		$limit_payparams = M('payparams_list')->where($where)->find();
		if(!empty($limit_payparams)){
			$tips['limit_payparams'] = '账户【'.$limit_payparams['truename'].'】可能已经限额，请检查后调整风控，取消提示状态为关闭一次！';
		}

		//最大失败次数提示
		unset($where['low_money_count']);
		unset($where['limit_amount_status']);
		$where['status'] = 0;
		$where['max_fail_count'] = ['egt', 10];
		$fail_count = M('payparams_list')->where($where)->count();
		if($fail_count > 0){
			$tips['fail_payparams'] = '有账户已经因连续失败次数过多而关闭，已标记为<b style="color:red">红色</b>，请检查后再开启，取消提示状态为开启一次！';
		}

		//有自动买入订单提示
		unset($where['max_fail_count']);
		unset($where['low_money_count']);
		unset($where['limit_amount_status']);
		$cur_time = time();
		$where['status'] = 1;
		$where['otype'] = 1;
		$where['addtime'] = ['between', [$cur_time-24*60*60, $cur_time]];
		$where['notifyurl'] = ['exp', 'is not null'];
		$auto_buy_count = D('ExchangeOrder')->where($where)->count();
		if($auto_buy_count > 0){
			$tips['auto_buy_order_exist'] = '有<b style="color:green">自动买入订单</b>，请及时处理, 可搜索快速查找！';
		}

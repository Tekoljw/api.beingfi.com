...
class VerifyController extends \Think\Controller
{
	...
	{
		$allow_action = array("code","code2","real","real1","regss","mytx","paypass","pass","mibao","mobilebd","findpwd","findpaypwd","myzc","tests","findpwd_test");
...
/*	public function tests()
	...
	public function regss($mobile, $intnum,$verify)
	{
		...
		session('mobileregss_verify', md5($code.'mima'));
		...
		$fh = sendsmsint($mobile, $code, $sign);
		if ($fh) {
...
/*		if (smssend($mobile, $content)) {
	...
	public function mobilebd($mobile, $verify) {
		...
		$content = "您正在进行手机绑定操作，您的验证码是:" . $code;
		$sign = "【".$config['smsqm']."】";
		$fh = sendsmsint($mobile, $code, $sign);
		if ($fh) {
			$this->success(L('短信验证码已发送到你的手机，请查收'));

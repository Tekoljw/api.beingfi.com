...
class RewardController extends HomeController
{
    ...
    {
		$this->promoteHandle(1);
    }

    // 充值佣金奖励处理（定时任务）
    public function RechargesHandle()
    {
		$this->promoteHandle(2);
    }
	
	// 交易佣金奖励处理（定时任务）
    public function CommissionHandle()
    {
		$this->promoteHandle(3);
    }

    //发送推广奖励
    private function promoteHandle($type){
		...
		foreach ($invit_data as $k => $v) {
			...
			if($v['fee'] > 0){
				...
				// 修改金额
				$rs[$k][] = $mo->table('tw_user_coin')->where($where_1)->setInc(strtolower($v['coin']), $v['fee']); 
			}
		...
		try{
			foreach ($MoneyList as $money) {
				...
				if($money["lasttime"]){
					...
					echo "项目名称：".$money["name"]."准备启动计时".$br;
				}

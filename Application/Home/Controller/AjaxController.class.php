...
class AjaxController extends HomeController
{
	...
	//上传企业资质图片
	public function imgOrganization()
	{
		if (!userid()) {
			echo "nologin";
		}
		...
		$upload->maxSize = 3145728;
		$upload->exts = array('jpg', 'gif', 'png', 'jpeg');
		$upload->rootPath = './Upload/organization_image/';
		$upload->autoSub = false;
		$info = $upload->upload();
		foreach ($info as $k => $v) {
			$path = $v['savepath'] . $v['savename'];
	...
	public function allfinance($ajax = 'json')
	{
		...
		foreach (C('coin') as $k => $v) {
			if ($v['name'] == Anchor_CNY) {
				...
				$market_coin = $v['name'].'_'.Anchor_CNY;
				if (isset($market[$market_coin]) && $market[$market_coin]['new_price']) {
					$jia = $market[$market_coin]['new_price'];
				} else {
					$jia = 1;
				}
...
/*		exit;
	...
	// C2C获取付款商户信息
	public function c2cPayment($orderid, $type, $ajax = 'json')
	{
		// 过滤非法字符----------------S
		if (checkstr($orderid)) {
			$this->error('您输入的信息有误！');

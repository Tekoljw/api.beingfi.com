...
class UserController extends HomeController
{
	...
	{
		parent::_initialize();
		$allow_action=array("index","login","nameauth","uppassword","paypassword","uppaypassword","uppaypasswordset","ga","mobile","upmobile","tpwdset","tpwdsetting","uptpwdsetting","bank","upbank","delbank","qianbao","upqianbao","delqianbao","goods","upgoods","delgoods","log","gaGoogle","kyc","kyc1","kyc2","kyc1_Handle","kyc2_Handle","kyc_api","kycOrganization","kycOrganization_Handle","apiManager","payTypeIntroduce");
	...
	public function index()
	{
		...
		$organization_status = $organization_status?$organization_status:0;
		$this->assign('organization_status', $organization_status);
...
/*		if (!$user['idcard']) {
	...
	public function kyc1_Handle($idnationality, $idtype, $truename, $idcard)
	{
		...
		if ($idnationality == '中国' || strtolower($idnationality) == 'china' || strtolower($idnationality) == 'cn') {
/*			if (!check($truename, 'truename')) {
				$this->error('真实姓名格式错误！');
			}*/
			if ($idtype == 1 && !check($idcard, 'idcard')) {
		 		$this->error(L('身份证号格式错误！'));
			}
			if(C('real_switch') == 1){ // 启动api自动认证
				$this->kyc_api($idcard,$truename); 
			}
		}
	...
	// API实名认证(type：0个人实名认证，1机构法人认证)
    public function kyc_api($cardno,$name,$type=0)
    {
		// 过滤非法字符----------------S
		...
		if($handlas['resp']['code'] || $handlas['resp']['code'] == 0){
			if($type == 1){
				M('user_kyc')->where(array('userid' => userid()))->save(array('organization_legalperson_status' => $handlas['resp']['code']));
			}else{
	...
	//机构认证
	public function kycOrganization()
	{
		if (!userid()) {
			redirect('/Login/index');
		}
		...
		$user = M('User')->where(array('id' => userid()))->find();
		$user_kyc = M('user_kyc')->where(array('userid' => userid()))->find();
		if ($user['kyc_lv'] != 2 || $user['idstate'] != 2) {
			$this->error(L('非法操作'), U('User/index'));
		} else if ($user_kyc['organization_status'] == 2) {
			$this->error(L('非法操作'), U('User/index'));
		}
	...
	public function kycOrganization_Handle($name, $org_id, $person, $person_id, $id_type, $idimg1, $idimg2, $idimg3, $idimg4, $idimg5)
	{
		// 过滤非法字符----------------S
		if (checkstr($name) || checkstr($org_id) || checkstr($person) || checkstr($person_id) || checkstr($id_type) ||checkstr($idimg1) || checkstr($idimg2) || checkstr($idimg3) || checkstr($idimg4) || checkstr($idimg5)) {
		...
		if ($user['kyc_lv'] != 2 || $user['idstate'] != 2) {
			redirect('/User/index');
		} else if($user_kyc && $user_kyc['organization_status'] == 2) {
			redirect('/User/index');
		}

		if(!$name || !$org_id){
			$this->error(L('请填写企业信息！'));
		}

		if(!$person || !$person_id || !$id_type){
			$this->error(L('请填写法人信息！'));
		}
		
		if(!$idimg1 && !$idimg2 && !$idimg3 && !$idimg4 && !$idimg5){
			$this->error(L('请上传证件照后再提交！'));
		}

		if(!check($org_id, 'business_license_id')){
			$this->error(L('企业信任代码格式错误！'));
		}

		//如果证件为身份证
		if ($id_type == 1) {

			if(!check($person, 'truename')) {
		 		$this->error(L('法人姓名格式错误！'));
			}
			if(!check($person_id, 'idcard')){
				$this->error(L('身份证号格式错误！'));
			}
		}
		...
		$data = array(
			...
			'organization_legalperson' 			=> $person,
			'organization_legalperson_id' 		=> $person_id,
			'organization_legalperson_idtype' 	=> $id_type,
			'organization_img1' 	=> $idimg1, 
			'organization_img2' 	=> $idimg2, 
			'organization_img3' 	=> $idimg3,
			'organization_img4' 	=> $idimg4,
			'organization_img5' 	=> $idimg5,
			'organization_time'		=> time(),
		);

		if($user_kyc){
			unset($data['userid']);
			$res = M('user_kyc')->where(array('userid' => userid()))->save($data);
		}else{
			$res = M('user_kyc')->add($data);
		}
		
		if($res){
			//自动身份证认证
			if(C('real_switch') == 1 && $id_type == 1){ // 启动api自动认证
				$this->kyc_api($person_id, $person, 1); 
			}
			$this->success(L('证件上传成功！'));
		} else {
			$this->error(L('证件上传失败！'));
	...
	public function ga()
	{
		if (empty($_POST)) {
			...
			if ($ga->verifyCode($secret, $gacode, 1)) {
				...
				$this->error(L('验证失败,请删除Google验证器记录,重新扫描或输入添加'));
			}
		}
	}

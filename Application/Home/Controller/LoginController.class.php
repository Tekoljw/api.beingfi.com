...
class LoginController extends HomeController
{
	...
	public function submit($username, $password, $verify = NULL, $ga='')
	{
		...
		if($user['ga']){
			...
			if($ga_is_login){
				if(!$ga){
					$this->error(L('请输入Google验证码！'));
				}
				if(!check($ga,'d')){
					$this->error(L('Google验证码格式错误！'));
				}
				// 判断登录有无验证码
				$aa = $ga_n->verifyCode($secret, $ga, 1);
				if (!$aa){
					$this->error(L('Google身份验证码错误！'));
				}
			}
		}
		// 处理谷歌身份验证器-------------------E
	...
	{
		//$this->error("注册系统升级，暂停注册！20:30开放注册",U('Login/index'));
...
/*		if (!check($enname, 'username')) {
		...
		$data = array(
			...
			'addr' 		=> get_city_ip(), 
			'addtime' 	=> time(), 
			'apikey' 	=> get_random_str(),
			'status' 	=> 1 , 
			'otcuser'	=> trim($mobile),
			'enname'	=> $enname,
			'qz'		=> $qz,
			'idcardinfo'=> '',
			'auto_c2c_sell_status' => 0,
			'auto_c2c_buy_status'  => 0
		);

		$rs = array();
		$rs[] = $mo->table('tw_user')->add($data);
	...
	public function findpaypwd()
	{
		if (IS_POST) {
			...
			if (check_arr($rs)) {
				...
				$mo->execute('rollback');
				$this->error(L('修改失败') . $mo->table('tw_user')->getLastSql());
			}
		} else {

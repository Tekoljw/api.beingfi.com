...
class QueueController extends HomeController
{
    ...
    public function checkDapan()
    {
        ...
        foreach (C('market') as $k => $v) {
            ...
            //处理周六周日是否可交易===结束
            R('Trade/matchingTradeall',[$v['name']]);//匹配交易
    ...
    {
        foreach (C('market') as $k => $v) {
            if (true){//(!$v['hou_price'] || (date('H', time()) == '0')) {
                $t = time();
                $start = mktime(0, 0, 0, date('m', $t), date('d', $t), date('Y', $t));
    ...
    //外网可以访问的函数
    public function outsideBackupWalletData()
    {
        return $this->backupWalletData();
    }

    //备份钱包数据
    private function backupWalletData(){
        ...
        $filename = '/home/backup/wallet/omnicore/backupwallet-'.date('m-d').'.dat';
        $result = $CoinClient->backupwallet($filename);
        if(is_null($result)){
            return true;
        }else{
            return false;
	...
	public function setTradeJson($market)
	{
        ...
        $marketTradeLog = M("TradeLog")->where(array("market" => $market))->order("id desc")->find();
        if(!$marketTradeLog){
            return '市场没有交易信息，不用计算！';
        }
		...
		foreach ($timearr as $k => $v) {
			...
			if ($addtime) {
				$haveTradeLog = M("TradeLog")->where("addtime >=" . $addtime . "  and market ='" . $market . "'")->sum("num");
			}

			if (!empty($haveTradeLog)) {
				if ($v == 1) {
					$start_time = $addtime;
				} else {
				...
				for ($x = 0; $x <= 20; $x++) {
                    ...
                    //查询条件
                    $where = array(
                        'addtime'   => ['between', [$na, $nb]],
                        'market'    => $market,
                    );

					$sum = M("TradeLog")->where($where)->sum("num");
					
					if ($sum) {
						$sta = M("TradeLog")->where($where)->order("id asc")->getField("price");
						$max = M("TradeLog")->where($where)->max("price");
						$min = M("TradeLog")->where($where)->min("price");
						$end = M("TradeLog")->where($where)->order("id desc")->getField("price");

						$trade_array = array($na, $sum, $sta, $max, $min, $end);//时间，成交量,成交价,最高价,最低价,收盘价
					    //交易数据的json
                        $trade_json = json_encode($trade_array);
                        if(!$trade_json){
                            $trade_json = '';
                        }
						...
						if ($times >= 1){
							$jiansuo = M("TradeJson")->where(array("market" => $market, "data" => $trade_json, "addtime" => $na, "type" => $v))->find();
							if ($jiansuo) {
								$sdfds = array();
								$sdfds['market'] = $market;
								$sdfds['price'] = $sta;
						...
						if (M("TradeJson")->where(array("market" => $market, "addtime" => $na, "type" => $v))->find()) {
							M("TradeJson")->where(array("market" => $market, "addtime" => $na, "type" => $v))->save(array("data" => $trade_json));
						} else {
							$aa = M("TradeJson")->add(array("market" => $market, "data" => $trade_json, "addtime" => $na, "type" => $v));
							M("TradeJson")->execute("commit");
            ...
            if ($youtradelog) {
                ...
                for (; $x <= $chas; $x++) {
                    ...
                    if ($sum) {
                        ...
                        //交易数据的数组
                        $trade_array = array($na, $sum, $sta, $max, $min, $end);
                        //交易数据的json
                        $trade_json = json_encode($trade_array);
                        if(!$trade_json){
                            $trade_json = '';
    ...
    //获取coinmarketcap的市场行情(默认取第1个，转换为CNY)
    private function httpMarketInfoWithCoinmarketcap($http_url, $start=1, $limit=1, $convert='CNY'){
        $url = $http_url;
        $parameters = [
          'start' => $start,
          'limit' => $limit,
        ...
        $headers = [
          ...
          'X-CMC_PRO_API_KEY: d3e7760b-683e-4682-97f4-28a206ea5aab'
        ];
        ...
        curl_setopt_array($curl, array(
          ...
          CURLOPT_TIMEOUT => 10,              // set request time out
          CURLOPT_CONNECTTIMEOUT => 30,       // set connection time out
	...
	// 行情爬虫 CoinMarketCap
    public function usdtToRMB()
    {
        $config = M('config')->where(array('id' => 1))->find();
        if(!is_null($config)){
            $http_url = 'https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest';
            $content = $this->httpMarketInfoWithCoinmarketcap($http_url, 1, 8); //5是标记为usdt
            if($content){
                $content = json_decode($content, true);
                if(is_array($content)){
                    var_dump($content);
                    //是否获取到usdt价格
                    $obtain_usdt_data = false;
                    $data = $content['data'];
                    foreach ($data as $key => $data_array) {
                        $usdt_flag = $data_array['symbol'];
                        if($usdt_flag == 'USDT'){ //必须为usdt才计算
                            $obtain_usdt_data = true;
                            $quote_data = $data_array['quote'];
                            $cny_data = $quote_data['CNY'];
                            $usdt = round($cny_data['price'], 2);
                            $usdtlast = $cny_data['last_updated'];

                            $map['usdt'] = $usdt - ($usdt * 0.0002); //价格做过额外调整
                            $map['usdtlast'] = $usdtlast;
                            if($map['usdt'] == $config['usdt']){ //价格相同不更新
                                unset($map['usdt']);
                            }
                            
                            if($map['usdtlast'] == $config['usdtlast']){
                                unset($map['usdtlast']);
                            }

                            if(!empty($map)){
                                //echo "同步CNY ： {$map['usdt']}";
                                $res = M('config')->where(array('id' => 1))->save($map);
                                if ($res) {
                                    echo "同步CNY的价格成功\n";
                                }else{
                                    echo "更新CNY的价格失败\n";
                    ...
                    if(!$obtain_usdt_data){
                        echo "获取货币价格的接口获取到不到USDT！ coin：{$usdt_flag}\n";
                        return "获取货币价格的接口获取到不到USDT！ coin：{$usdt_flag}";
                    }
                }else{
                    echo "获取货币价格信息失败11！\n";
                    return '获取货币价格信息失败11！';
    ...
    public function TestCoinMarket()
    {
        $url = 'https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest';
        $content = file_get_contents_new($url, 5);
        $content = json_decode($content, true);
        echo($content);
    }
	
	// 行情爬虫 火币网
    public function PriceFromfireCoinWeb()
    {
		$config = M('config')->where(array('id' => 1))->find();
        $usdtToRmbPrice = $config['usdt'];
		...
		foreach ($marketData as $k => $v) {
            ...
            $wei = 10;
            if($xnb[$k] == 'usdt' && $rmb[$k] == Anchor_CNY){ //原货币就是usdt
                $min_price = ($usdtToRmbPrice - ($usdtToRmbPrice * 0.003)) * $wei; //最低价格
                $max_price = ($usdtToRmbPrice + ($usdtToRmbPrice * 0.0002)) * $wei; //最高价格
            }else{
                $url = 'https://api.huobi.pro/market/detail/merged?symbol='.$xnb[$k].$rmb2[$k];
                $content = file_get_contents_new($url, 5);
                $content = json_decode($content, true);
                if(is_array($content)){
                    $tick = $content['tick'];
                    if ($rmb[$k] == Anchor_CNY) {
                        $tick['low'] = $tick['low'] * $usdtToRmbPrice; //卖一价
                        $tick['high'] = $tick['high'] * $usdtToRmbPrice; //本阶段最高价
                        $tick['close'] = $tick['close'] * $usdtToRmbPrice; //当前价（close最新价，high本阶段最高价）

                        $high_price = $tick['high'] * $wei; //能够达到的最大价格
                    }

                    $tick['close'] = $tick['close'] *(1+0.001); //调整最高价

                    $min_price = $tick['low'] * $wei; //可以出价的最低价格
                    $max_price = $tick['close'] * $wei; //可以出价的最高价格
                    $high_price = $high_price?$high_price:$max_price;

                    if($high_price > 0 && $max_price > $high_price){
            ...
            if($v['sdlow'] == $map['sdlow']){ //价格相同不更新
                unset($map['sdlow']);
            }
            if($v['sdhigh'] == $map['sdhigh']){//价格相同不更新
                unset($map['sdhigh']);
            }

            if(!empty($map)){
                //echo $v['name']." - sdlow:{$map['sdlow']},sdhigh:{$map['sdhigh']},url:{$url} \n";
                if (M('market')->where(array('name' => $v['name']))->save($map)) {
                    if (!isset($map['sdlow'])) {
                        echo $v['name']." - 最低价相同不更新\n";
                    }elseif(!isset($map['sdhigh'])){
                        echo $v['name']." - 最高价相同不更新\n";
                    } else {
                        echo $v['name']." - OK\n";
                    }
                } else {
    ...
    public function autojy2($market = NULL)
    {
        ...
        if ($marketinfo['shuadan'] == 1) {//开启刷单则进行
            ...
            //基础货币市场
            $base_market = 'usdt_'.Anchor_CNY;
            //如果是卖出订单则可以高出最高价,除了usdt->cny的价格
            if($type == 2 && $market != $base_market)
            {
                $max_price = $max_price * (1+0.03);
            }
			
            //如果设置了最高,最低涨跌上下线,则价格按照此区域运行
...
/*				if ($muls >= 10000 || $muns >= 10000) {
            ...
            if (!$price) { return '交易价格格式错误';}
            if (!check($num, 'double')) { return '交易数量格式错误';}
            if (($type != 1) && ($type != 2)) { return '交易类型格式错误';}
            if (!$price) { return '交易价格错误'; }
			
            $num = round(trim($num), 5);
            if (!check($num, 'double')) { return '交易数量错误'; }
			
            $mum = round($num * $price, 8);
            if (!$rmb) { return '数据错误1';}
            if (!$xnb) { return '数据错误2';}
            if (!$market) { return '数据错误3';}
            if (!$price) { return '数据错误4';}
            if (!$num) { return '数据错误5';}
            if (!$mum) { return '数据错误6';}
            if (!$type) { return '数据错误7';}
			
            $mo = M();
            $mo->execute('set autocommit=0');
            $mo->execute('lock tables tw_trade write ');
            $rs = array();
			if ($price <= 0 || $num <= 0 || $mum <= 0) {return '交易失败！';}
            if ($type == 1) {
            ...
            if (check_arr($rs)) {
                ...
                if(R('Home/Trade/matchingTradeall',[$market]) !== false){ //匹配所有订单
                    return true;
                }else{
                    return '交易匹配订单失败！';
                }
    ...
    function etconline($coin = 'etc')
    {
        ...
        $fanhui = file_get_contents_new($url);
    ...
    /** 同步钱包转入记录 **/
    public function syncWalletInfo()
    {
        ...
        foreach ($coinList as $k => $v) {
            ...
            switch (strtolower($coin)) {
                ...
                case 'etc':
                    ...
                    break;
                default:
                    $this->normalCoinEnterInto($coin, $coinid);
                    break;
            }
        }
        return true;
    ...
    //通用货币入账
    private function normalCoinEnterInto($coin, $coinid){
	...
	/** USDT入账 **/
    private function usdtEnterInto()
    {
        ...
        foreach ($omnilist as $v) {
            ...
            if ($v['confirmations'] < 3) {
                ...
                echo '确认次数为'. $v['confirmations'] . '次未达标' . "\n";
                continue;
            } else {
                echo '确认次数达到3次,完成.' . "\n";
            }
    ...
    /** ETH入账 **/
    private function ethonlinea88b77c11d0a9d($coin = 'eth')
    {
        set_time_limit(0);
        ignore_user_abort();
        $dj_username = C('coin')[$coin]['dj_yh'];
        ...
        $dj_address = C('coin')[$coin]['dj_zj'];
        $dj_port = C('coin')[$coin]['dj_dk'];
        $ethModel = EthCommon($dj_address, $dj_port);
        $accounts = $ethModel->personal_listAccounts();//获取钱包地址列表
                        ...
                        if ($fanhui['message'] == 'OK') {
                            foreach ($fanhui['result'] as $v2) {
                                if ($v2['to'] == $v && $v2['txreceipt_status'] == 1) {
                                    $rs1 = M()->table('tw_myzr')->where(array('txid' => $v2['hash']))->find();
                                    if (!$rs1) {
                                        $amount = $v2['value'] / $ethModel->base_wei();
                                        $rs2 = M()->table('tw_myzr')->add(array('userid' => $getdz['userid'], 'username' => $v, 'coinname' => $coin, 'fee' => 0, 'txid' => $v2['hash'], 'num' => $amount, 'mum' => $amount, 'addtime' => time(), 'status' => 1));
    ...
    /** ETC入账 **/
    private function etconlinea88b77c11d0a9d($coin = 'etc')
    {
        $dj_username = C('coin')[$coin]['dj_yh'];
        $dj_password = C('coin')[$coin]['dj_mm'];
        $dj_address = C('coin')[$coin]['dj_zj'];
...
/*		if($coin=='fff'){
        ...
        $blockn= file_get_contents_new($getblock);

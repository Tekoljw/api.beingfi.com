...
class TradeController extends HomeController
{
	public function index($market = NULL)
	{
		...
		$sclj = explode('_', $market)[1]; //获取兑换的货币类型
		$coinnav = 0;

		switch ($sclj) {
			case Anchor_CNY: 	//锚定法币
				$rmb = bcdiv($topdata['info']['new_price'] * C('MYCOIN'),1,2) * 1;
				$coinnav = 2; 	//CNC市场
				break;
			case 'btc':
				$rmb = bcdiv($topdata['info']['new_price'] * C('market')['btc_'.Anchor_CNY]['new_price'],1,2) * 1;
				$coinnav = 1;	//btc市场
				break;
			case 'usdt':
	...
	//今日是否能够进行交易
	private function isCanTodayUpTrade($market, $type, $kyc_lv){
		if($market){
			//机构认证状态
			$organization_status = M('user_kyc')->where(array('userid' => userid()))->getField('organization_status');
			if(!$organization_status){ //机构认证不通过则限制

				$todyNum = M('trade')->where(['userid'=> userid(), 'market'=>$market, 'addtime'=> ['gt', strtotime(date("Y-m-d 00:00:00"))]])->sum('num');

				$todyNum = $todyNum?$todyNum:0;

				$xnb[$k] = explode('_', $market)[0];
				$rmb[$k] = explode('_', $market)[1];
				if($type == 1){ //买
					
					if($rmb[$k] == Anchor_CNY){
						$new_price = C('market')[$market]['new_price'];
					}else{
						$market = $xnb[$k].'_'.Anchor_CNY;
						$new_price = C('market')[$market]['new_price'];
					}
				}else{ //卖
					if($rmb[$k] == Anchor_CNY){
				...
				switch ($kyc_lv) {
					case 1:
						if($new_price && ($todyNum*$new_price/$usdt_new_price) < C('TODAY_TRADE_KYC_LIMIT')){ //5万美金
							return true;
						}
						break;
					case 2:
	...
	public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		...
		//平台币交易
		if(strpos($market, Anchor_CNY) !== false){
			if($user['auto_c2c_sell_status']){
				$this->error(L('开启自动卖出状态下无法进行'.strtoupper(Anchor_CNY).'交易'));
			}
		}
		...
		//判断是否达到认证的限制金额
		if(!$this->isCanTodayUpTrade($market, $type, $user['kyc_lv'])){

			$this->error(L('超过今日交易上限'.C('TODAY_TRADE_KYC_LIMIT').'美金，进行机构认证可以打开限制！'));
		}
	...
	public function matchingTradeall($market = NULL)
    {
					...
					if ($invit_1) {
						if ($buy_fee) {
							if ($invit_buy_user['invit_1']) {
								...
								if ($invit_buy_save_1) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_buy_user['invit_1'], 'invit' => $buy['userid'], 'name' => '一代买入赠送', 'type' => 4, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_1, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
							...
							if ($invit_buy_user['invit_2']) {
								...
								if ($invit_buy_save_2) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_buy_user['invit_2'], 'invit' => $buy['userid'], 'name' => '二代买入赠送', 'type' => 4, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_2, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
							...
							if ($invit_buy_user['invit_3']) {
								...
								if ($invit_buy_save_3) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_buy_user['invit_3'], 'invit' => $buy['userid'], 'name' => '三代买入赠送', 'type' => 4, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_3, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
						}
					}
						...
						if ($sell_fee) {
							if ($invit_sell_user['invit_1']) {
								...
								if ($invit_sell_save_1) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_sell_user['invit_1'], 'invit' => $sell['userid'], 'name' => '一代卖出赠送', 'type' => 5, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_1, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
							...
							if ($invit_sell_user['invit_2']) {
								...
								if ($invit_sell_save_2) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_sell_user['invit_2'], 'invit' => $sell['userid'], 'name' => '二代卖出赠送', 'type' => 5, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_2, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
							...
							if ($invit_sell_user['invit_3']) {
								...
								if ($invit_sell_save_3) {
									$rs[] = $mo->table('tw_invit')->add(array('userid' => $invit_sell_user['invit_3'], 'invit' => $sell['userid'], 'name' => '三代卖出赠送', 'type' => 5, 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_3, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($rmb1)));
								}
							}
						}
					}
		...
		if ($new_trade_btchanges) {
			...
			// 计算涨跌幅
			$hou_price = $Cmarket['hou_price']; //之前的价格
			if($hou_price && $hou_price != 0 && $new_price != $hou_price){
    ...
    {
        //删除超过6小时未成交的虚假订单
        $map['addtime'] = array('lt', (time() - 6 * 60 * 60));
        $map['userid'] = 0; //userid = 0 表示机器人订单
        $map['status'] = 0;
        $deldd = M('trade')->where($map)->limit(20)->delete();
        if ($deldd) {
            echo '已设置' . $deldd . "条币币交易机器人数据为删除状态\n";
        } else {
            echo "没有可以清除的币币交易机器人数据\n";
        }
        return true;
    }

    //清除6小时内没有交易的所有机器人订单
    public function overtimeDeleteAll()
    {
        //删除超过6小时未成交的虚假订单
        $map['addtime'] = array('lt', (time() - 6 * 60 * 60));
        $map['userid'] = 0; //userid = 0 表示机器人订单
        ...
        if ($deldd) {
            echo '设置所有超时的' . $deldd . "条币币交易机器人数据为删除状态\n";
        } else {
            echo "没有可以清除的币币交易机器人数据\n";
        }
        return true;
    ...
    //清除超过2个月的币币交易成功日志
    public function beforeMonthTradeJsonDelete()
    {
    	//删除超过2个月的币币交易成功日志
        $map['addtime'] = array('lt', (time() - 60*24*60*60));
        $deldd = M('TradeJson')->where($map)->delete();
        if ($deldd) {
            echo '删除币币交易之前月的' . $deldd . "条交易成功json日志\n";
        } else {
            echo "没有可以清除的交易成功json日志数据\n";
        }
        return true;
    }
    
    //刷新每日支付金额
    public function refreshPayingMoney()
    {
        $upRes = M('payparams_list')->where("1=1")->save(['paying_money' => 0]);
        if ($upRes) {
            echo "已完成刷新每日收款金额\n";
        } else {
            echo "每日收款金额刷新失败\n";

...
class CoinClient
{
	...
	private function doRequest(array $payload)
	{
		...
		//Log::record("==> Request".json_encode($payload, JSON_PRETTY_PRINT), Log::INFO);
		//Log::record("==> Response".json_encode($response, JSON_PRETTY_PRINT), Log::INFO);
		...
		if ($code == '200') {
			...
			Log::record('SERVER 111返回' . $code . '[' . $detail . ']', Log::INFO);

			$this->error('SERVER 111返回' . $code . '[' . $detail . ']');
		} else {
			Log::record('SERVER 222返回' . $code, Log::INFO);
			$this->error('SERVER 222返回' . $code);
		}
	}

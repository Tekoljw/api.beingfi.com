...
function excludeKey($key){
    ...
    //通知地址,网关,签名 排除
    if(strpos($tempKey, "notifyurl") !== false || strpos($tempKey, "callbackurl") !== false || strpos($tempKey, "gateway") !== false || strpos($tempKey, "pay_proof") !== false || strpos($tempKey, "sign") !== false){
        return true;
    }

    //特殊字段排除(必须为小写字符串)
    $allow_key=array("content",'serverreturn','pagereturn','noticestr','appsecret','nonce_str', 'openid', 'fund_bill_list','buyer_logon_id', 'seller_email');
    
    if(in_array($tempKey,$allow_key)){
       return true; 
    }
    return false;
}
...
//不用检查的str
function excludeStr($str){
    //转换为小写
    $tempstr = strtolower($str);
    //特殊字段排除
    $allow_str=array("unionpaywap","unionpaywgmobile","unionpaywgpc","unionpaysdk","leunion");
    
    foreach ($allow_str as $key => $value) {
        if(strpos($tempstr, $value) !== false){
           return true;
        }
    }
    return false;
...
function wjSec(&$array) {
    if (is_array($array)) {
        foreach ($array as $k => $v) {
            if (($k == 'addr' && strlen($v) == 34) || excludeKey($k)) {
                $array [$k] = $v;
            } else {
                $array [$k] = wjSec($v);
            }

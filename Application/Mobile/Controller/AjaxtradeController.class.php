...
class AjaxtradeController extends MobileController
{
	...
	public function ajax_trade($coin_name = null)
	{
		...
		if ($userCoin) {
			...
			$data['usercoin']['cny'] = sprintf("%.3f", floatval($userCoin[$rmb]) - floatval($userCoin['shouyi_num']));
			$data['usercoin']['cnyd'] = sprintf("%.3f",floatval($userCoin[$rmb . 'd']) - floatval($userCoin['shouyi_numd']));

 ...
 * @date   2018-06-06
 */
//获取RM支付宝二维码QR类型的自动通知控制器(文档：https://doc.revenuemonster.my/#payment-web-app-checkout)
class PayRmAlipayQRAutoNotifyController extends BaseController
{
    /**
     * 初始化控制器
    ...
    public function requestOrder($payParams, $postInfo)
    {
        if($payParams && $postInfo){
            ...
            $redirectUrl = C('STAR_EXCHANGE_WEB_URL')."/Pay/PayRmAlipayQRAutoNotify/callbackurl.html?orderid={$payParams['orderid']}";
            $notifyurl = C('STAR_EXCHANGE_WEB_URL').'/Pay/PayRmAlipayQRAutoNotify/notifyurl.html';
            ...
            if($result && strpos($result, "httpcode") === false){
                ...
                if($resultArray['code'] == 'SUCCESS'){
                    ...
                    return $this->returnErrorMsg('PayRmAlipayQRAutoNotify requestOrder 请求参数错误code = '. $resultArray['code']);
                }
            }else{

                return $this->returnErrorMsg('PayRmAlipayQRAutoNotify requestOrder 请求参数错误result = '. $result);
            }
        }else{
            return $this->returnErrorMsg('PayRmAlipayQRAutoNotify requestOrder 请求参数无效');
        }

 ...
 * @date   2018-06-06
 */
//获取RM微信类型的自动通知控制器(文档：https://doc.revenuemonster.my/#payment-web-app-checkout)
class PayRmWxAutoNotifyController extends BaseController
{
    /**
     * 初始化控制器
    ...
    public function requestOrder($payParams, $postInfo)
    {
        if($payParams && $postInfo){
            ...
            $redirectUrl = C('STAR_EXCHANGE_WEB_URL')."/Pay/PayRmWxAutoNotify/callbackurl.html?orderid={$payParams['orderid']}";
            $notifyurl = C('STAR_EXCHANGE_WEB_URL').'/Pay/PayRmWxAutoNotify/notifyurl.html';

            //请求数据
            $requestdata = [
                'order'         => $order,
                'method'        => ['WECHATPAY_CN'], //微信
            ...
            if($result && strpos($result, "httpcode") === false){
                ...
                if($resultArray['code'] == 'SUCCESS'){
                    ...
                    return $this->returnErrorMsg('PayRmWxAutoNotify requestOrder 请求参数错误code = '. $resultArray['code']);
                }
            }else{

                return $this->returnErrorMsg('PayRmWxAutoNotify requestOrder 请求参数错误result = '. $result);
            }
        }else{
            return $this->returnErrorMsg('PayRmWxAutoNotify requestOrder 请求参数无效');
        }
    ...
    private function requestQRInfo($payParams, $token, $checkout_id, $payurl){
        ...
        $url = "https://open.revenuemonster.my/v3/payment/online/qrcode?checkoutId={$checkout_id}&method=WECHATPAY_CN";
        $notic_str = get_random_str();
        $time = time();
        ...
        if($result && strpos($result, "httpcode") === false){
            ...
            if($resultArray['code'] == 'SUCCESS'){
                ...
                return $this->returnErrorMsg('PayRmWxAutoNotify requestQRInfo 请求参数错误error result = '. $result);
            }
        }else{
            return $this->returnErrorMsg('PayRmWxAutoNotify requestQRInfo 请求参数无效invaild result = '.$result);
        }
    }

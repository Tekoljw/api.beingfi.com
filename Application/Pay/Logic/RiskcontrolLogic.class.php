...
class RiskcontrolLogic extends \Think\Model
{
    ...
    //监测商户单个用户单日单通道单子账号的数据
    public function monitoringMemberUserChannelAccountData($member_userid,$channel_account_id,$channel_id)
    {
        //该配置存在单日单商户的用户的风控数据
        if($this->config_info && isset($this->config_info['single_member_money'])){

            //单用户风控的数据
            $singleLimiteConfigMoney = $this->config_info['single_member_money'];
            $singleLimiteConfigNumber = $this->config_info['single_member_number'];

            if ($singleLimiteConfigMoney > 0 || $singleLimiteConfigNumber > 0){
                
                //对应数据
                $where = ['member_userid' => $member_userid, 'channel_account_id' => $channel_account_id];
                $data = M('channel_single_member')->where($where)->find();

                return $this->singleMemberUserChannelAccount($data, $where, $channel_id);
            }

            //Log::record('monitoringMemberUserChannelAccountData member_userid = '.$member_userid.' single_member_money = '.$this->config_info['single_member_money'], Log::INFO);
        }
        return true;
    }
    ...
    protected function scopeOfAmount()
    {
        ...
        if ($min_money != 0.00 && $max_money != 0.00) {
            ...
            if ($this->pay_amount < $min_money || $this->pay_amount > $max_money) {
                return '单笔交易金额需[' . $min_money . '元 —— ' . $max_money . '元]之间';
            }
        } else if ($min_money == 0.00 && $max_money != 0.00) {
            //判断用户交易的金额是否在交易范围
            if ($this->pay_amount > $max_money) {
                return '单笔交易最大金额[' . $max_money . '元]';
            }
        } else if ($min_money != 0.00 && $max_money == 0.00) {
            //判断用户交易的金额是否在交易范围
            if ($this->pay_amount < $min_money) {
                return '单笔交易最小金额[' . $min_money . '元]';
            }
        }
        return true;
    }
    ...
    protected function theTotalVolume(callable $callback)
    {
        if ($this->config_info['all_money'] != 0.00) {
            ...
            if ($now_date <= $last_paying_time) {
                //Log::record("theTotalVolume  now_date = ".$now_date.' last_paying_time = '.$last_paying_time, Log::INFO);
    ...
    {   
        ...
        if (array_key_exists('unit_interval', $this->config_info)) {

            $unit_interval = 0;
            $unit_interval_config = intval($this->config_info['unit_interval']);
            if($unit_interval_config <= 0){
                return true;
            }
            //计算时间单位
            switch ($this->config_info['time_unit']) {
                case 's': //秒
                    $unit_interval += $unit_interval_config;
                    break;
                case 'i': //分
                    $unit_interval += $unit_interval_config * 60;
                    break;
                case 'h': //时
                    $unit_interval += $unit_interval_config * 3600;
                    break;
                case 'd': //天
                    $unit_interval += $unit_interval_config * 86400;
                    break;
            }
            ...
            if ($time_lag <= $unit_interval) {
                ...
                if ($this->config_info['unit_all_money'] != '0.00') {
                    ...
                    if ($this->config_info['unit_frist_paying_time'] <= 0 && $this->pay_amount > $this->config_info['unit_all_money']) {
                        return '已超过单位时间总交易金额：' . $this->config_info['unit_all_money'].'元';
                    }
                    ...
                    if ($unit_paying_amount > $this->config_info['unit_all_money']) {
                        return '单位时间总交易金额：' . $this->config_info['unit_all_money'].'元';
                    }
                }

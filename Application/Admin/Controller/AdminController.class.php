...
//        var_dump($_POST['web_reg']);
    ...
    public function __construct()
    {
        ...
        if (!session('admin_id')) {
            //传递了urlkey，或者没有定义ADMIN_KEY
            if(in_array('urlkey', $_GET)){

                $this->redirect('Admin/Login/index',array('urlkey'=>$_GET['urlkey']));
            }else{
    ...
    //添加转出手续费
    protected function add_my_zc_fee($myzc, $fee_user, $user_coin, $coin, $type = 1){

        return M('myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $user_coin[$coin.'b'], 'coinname' => $coin, 'txid' => md5($myzc['username'] . $user_coin[$coin . 'b'] . time()), 'num' => $myzc['num'], 'fee' => $myzc['fee'], 'mum' => $myzc['mum'], 'type' => 2, 'addtime' => time(), 'status' => 1));
    }
    ...
    // 充值记录导出表处理
    protected function cz_exportExcel($expTitle, $expCellName, $expTableData)
    {
        import('Org.Util.PHPExcel');
        import('Org.Util.PHPExcel.Writer.Excel5');
        import('Org.Util.PHPExcel.IOFactory.php');
    ...
    // 用户表导出处理
    protected function us_exportExcel($expTitle, $expCellName, $expTableData)
    {
        import('Org.Util.PHPExcel');
        import('Org.Util.PHPExcel.Writer.Excel5');
        import('Org.Util.PHPExcel.IOFactory.php');
    ...
    //添加C2C订单资金变动日志
    protected function addC2COrderFinanceLog($orderInfo, $amount, $userid, $remark, $plusminus = 0, $optype = 0){
        ...
        $mo->execute('lock tables tw_user_coin write,tw_finance write,tw_finance_log write');

        $coin_type      = $orderInfo['type']; //提现类型
        $coin_type_d    = $orderInfo['type'].'d'; //提现类型，冻结
        $num            = isset($amount)?$amount:0;
         ...
         * 操作数据 amount
         */
        $rs[] = $mo->table('tw_finance_log')->add(array('username' => $username, 'adminname' => $username, 'addtime' => time(), 'plusminus' => $plusminus, 'amount' => $num, 'optype' => $optype, 'position' => 0, 'cointype' => 1, 'old_amount' =>  $old_coin_num, 'new_amount' => $new_coin_num, 'userid' => $userid, 'adminid' => $userid,'addip'=>get_client_ip()));
         // 处理资金变更日志-----------------E

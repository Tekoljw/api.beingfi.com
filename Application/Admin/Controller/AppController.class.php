...
class AppController extends AdminController
{
	...
	public function vip_config_edit()
	{
		if (empty($_POST)) {
			...
			operation_log(UID, 1, "编辑vipconfig");
	...
	public function vip_config_edit_status()
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "设置vip的状态禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "设置vip的状态开启");
			break;

		case 'delete':
			operation_log(UID, 1, "设置vip删除");
	...
	public function adsblock_edit_status()
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "设置广告板块的状态禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "设置广告板块的状态开启");
			break;

		case 'delete':
			operation_log(UID, 1, "删除广告板块");
			if (D('Appadsblock')->where($where)->delete()) {
				$this->success('操作成功！');
			}
			else {
	...
	public function ads_edit()
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				operation_log(UID, 1, "编辑app广告");
				$rs = D('Appads')->save($_POST);
			}
			else {
				operation_log(UID, 1, "添加app广告");
				$_POST['adminid'] = session('admin_id');
	...
	public function ads_edit_status()
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "app广告状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "app广告状态设置开启");
			break;

		case 'delete':
			operation_log(UID, 1, "app广告删除");
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('tw_user ON tw_user.id = tw_app_vipuser.uid')->join('tw_app_vip ON tw_app_vip.id = tw_app_vipuser.vip_id')->field('tw_user.username,tw_app_vipuser.*,tw_app_vip.name as vip_name,tw_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('tw_user ON tw_user.id = tw_app_vipuser.uid')->join('tw_app_vip ON tw_app_vip.id = tw_app_vipuser.vip_id')->field('tw_user.username,tw_app_vipuser.*,tw_app_vip.name as vip_name,tw_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);

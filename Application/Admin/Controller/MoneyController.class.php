...
class MoneyController extends AdminController
{
	...
	public function save()
	{
		...
		if ($_POST['outfee'] && (($_POST['outfee'] < 0.01) || (500 < $_POST['outfee']))) {
			$this->error('活期取现手续费范围为0.01 -- 500 %！');
		}
		if (floatval($_POST['num']) < floatval($_POST['deal'])) {
			$this->error('理财总量不能小于已理财量');
		}
		if (floatval($_POST['stnum'] <= 0)) {
			$this->error('最低起存数量不能小于0');
		}
		if (!floatval($_POST['tian'])) {
			$this->error('理财周期不能为空');
		}
		if ($_POST['type'] == 2 && !floatval($_POST['jian'])) {
			$this->error('利息释放间隔不能为空');
		}
		if (($_POST['fee'] < 0.01) || (500 < $_POST['fee'])) {
			$this->error('周期利息范围为0.01 -- 500 %！');
		}

		if (strtotime($_POST['addtime']) != strtotime(addtime(strtotime($_POST['addtime'])))) {
			$this->error('开始时间格式错误！');
		}
		...
		switch ($_POST['jiandanwei']) {
			case 'y':
				$_POST['jianstep'] = $_POST['jian'] * 12 * 30 * 24 * 60 * 60;
				break;

			case 'm':
				$_POST['jianstep'] = $_POST['jian'] * 30 * 24 * 60 * 60;
				break;

			case 'd':
				$_POST['jianstep'] = $_POST['jian'] * 24 * 60 * 60;
				break;

			case 'h':
				$_POST['jianstep'] = $_POST['jian'] * 60 * 60;
				break;

			default:

			case 'i':
				$_POST['jianstep'] = $_POST['jian'] * 60;
				break;
		}
		
		if ($_POST['id']) {
			operation_log(UID, 1, "理财配置编辑");
			$rs = $this->Model->where(array('id'=>$_POST['id']))->save($_POST);
		} else {
			operation_log(UID, 1, "理财配置添加");
	...
	public function log($money_id = NULL, $name = NULL)
	{
		if ($name && check($name, 'username')) {
			...
			$where = array(array('money_id' => $money_id));
		}
	...
	public function status()
	{
		...
		switch (strtolower($method)) {
			case 'forbid':
				$data = array('status' => 0);
				operation_log(UID, 1, "理财配置状态设置禁止");
				break;

			case 'resume':
				$data = array('status' => 1);
				operation_log(UID, 1, "理财配置状态设置开启");
				break;

			case 'delete':
				operation_log(UID, 1, "理财配置删除");

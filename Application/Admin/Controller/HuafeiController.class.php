...
class HuafeiController extends AdminController
{
	...
	public function delete($id = NULL)
	{
		...
		operation_log(UID, 1, "花费配置状态设置删除");
	...
	public function repeal($id = NULL)
	{
		...
		$mo->execute('lock tables  tw_user_coin write  , tw_huafei write ');
		$rs = array();
		$user_coin = $mo->table('tw_user_coin')->where(array('userid' => $huafei['userid']))->find();
		...
		$rs[] = $mo->table('tw_user_coin')->where(array('userid' => $huafei['userid']))->setInc($huafei['type'], $huafei['mum']);
		$rs[] = $mo->table('tw_huafei')->where(array('id' => $id))->save(array('endtime' => time(), 'status' => 2));

		operation_log(UID, 1, "花费配置状态设置废除");
		if (check_arr($rs)) {
			$mo->execute('commit');
	...
	public function resume($id = NULL)
	{
		...
		operation_log(UID, 1, "花费配置状态设置开启");
	...
	public function config()
	{
		...
		if (!in_array('huafei_appkey', $Config_DbFields)) {
			M()->execute('ALTER TABLE `tw_config` ADD COLUMN `huafei_appkey` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_openid', $Config_DbFields)) {
			M()->execute('ALTER TABLE `tw_config` ADD COLUMN `huafei_openid` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('huafei_zidong', $Config_DbFields)) {
			M()->execute('ALTER TABLE `tw_config` ADD COLUMN `huafei_zidong` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (empty($_POST)) {
			$this->display();
		}
		operation_log(UID, 1, "花费配置编辑");
	...
	{
		operation_log(UID, 1, "花费类型状态设置禁止");
	...
	{
		operation_log(UID, 1, "花费类型状态设置开启");
	...
	public function forbidCoin($id = NULL)
	{
		...
		operation_log(UID, 1, "花费币种状态设置禁止");
	...
	public function resumeCoin($id = NULL)
	{
		...
		operation_log(UID, 1, "花费币种状态设置开启");
	...
	public function deleteCoin($id = NULL)
	{
		...
		operation_log(UID, 1, "花费币种状态设置删除");

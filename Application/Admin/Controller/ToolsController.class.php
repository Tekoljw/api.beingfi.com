...
class ToolsController extends AdminController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","cache","delcahe","getDirSize","queue","qianbao","delcache","invoke","rmdirr","excel","export","import","clearData","operateRecord");
	...
	public function repair($tables = NULL)
	{
		...
		if ($tables) {
			...
			if (is_array($tables)) {
				...
				operation_log(UID, 1, "修复数据库表：".$tables);
				$list = $Db->query('REPAIR TABLE `' . $tables . '`');
    ...
    {   
		...
		if ($starttime && !$endtime) {
			$starttime = strtotime($starttime);
		  	$where['addtime'] = ['egt', $starttime];
		}elseif (!$starttime && $endtime) {
			$endtime = strtotime($endtime);
		  	$where['addtime'] = ['elt', $endtime];
		}elseif($starttime && $endtime){
			$starttime = strtotime($starttime);
			$endtime = strtotime($endtime);
			$where['addtime'] = ['between', [$starttime, $endtime]];
		}
		$count = M('operation_log')->where($where)->count();
		$Page = new \Think\Page($count, 30);
		$show = $Page->show();
		$list = M('operation_log')
		  ->where($where)
    ...
    public function clearData() {
        ...
        if(IS_POST) {
            ...
            $type_ids = $data['type'];
            if(empty($type_ids)){
                $this->error('请选择需清除的数据类型');
            }
            $starttime  = urldecode(I("request.starttime"));
            $endtime  = urldecode(I("request.endtime"));
            if ($starttime || $endtime) {
                $startTime = strtotime($starttime);
                $endTime = strtotime($endtime);
            ...
            foreach($type_ids as $v) {
                if($v == 1) { //交易记录
                    $res1 = D('trade_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }elseif($v == 2) {//用户记录
                    $res2 = M('user_log')->where(['addtime'=>['between',  [$startTime, $endTime]]])->delete();
                }elseif($v == 3) {//理财结算记录
                	$res3 = M('money_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                    $res4 = M('money_dlog')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }elseif($v == 4) {//认购记录
                    $res5 = M('issue_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }elseif($v == 5) {//资金记录
                    $res6 = M('finance_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }elseif($v == 6) {//操作记录
                    $res7 = M('operation_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }elseif($v == 7) {//分红记录
                    $res8 = M('fenhong_log')->where(['addtime'=>['between', [$startTime, $endTime]]])->delete();
                }
            }
            if(FALSE !== $res1 && FALSE !== $res2 && FALSE !== $res3 && FALSE !== $res4 && FALSE !== $res5 && FALSE !== $res6 && FALSE !== $res7 && FALSE !== $res8) {
                M()->commit();
                operation_log(UID, 1, "清理系统记录数据：".json_encode($type_ids));
                $this->success("清理成功！");
            } else {
                M()->rollback();
                $this->error("清理失败！");

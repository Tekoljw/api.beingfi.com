...
class ArticleController extends AdminController
{
	...
	public function status($id = NULL, $type = NULL, $mobile = 'Article')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "文章状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "文章状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "文章状态设置废除");
			break;

		case 'delete':
			$data = array('status' => -1);
			operation_log(UID, 1, "文章状态设置删除");
			break;

		case 'del':
			operation_log(UID, 1, "文章删除");
	...
	public function typeStatus($id = NULL, $type = NULL, $mobile = 'ArticleType')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "栏目类型状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "栏目类型状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "栏目类型状态设置废除");
			break;

		case 'delete':
			$data = array('status' => -1);
			operation_log(UID, 1, "栏目类型状态设置删除");
			break;

		case 'del':
			operation_log(UID, 1, "栏目类型删除");
			if (M($mobile)->where($where)->delete()) {
				$this->success('操作成功！');
			} else {
				$this->error('操作失败！');
	...
	public function adverEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				operation_log(UID, 1, "广告编辑");
				$rs = M('Adver')->save($_POST);
			} else {
				operation_log(UID, 1, "广告添加");
				//$_POST['adminid'] = session('admin_id');
	...
	public function adverStatus($id = NULL, $type = NULL, $mobile = 'Adver')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "广告状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "广告状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "广告状态设置废除");
			break;

		case 'delete':
			$data = array('status' => -1);
			operation_log(UID, 1, "广告状态设置删除");
			break;

		case 'del':
			operation_log(UID, 1, "广告直接删除");
			if (M($mobile)->where($where)->delete()) {
				$this->success('操作成功！');
			} else {
				$this->error('操作失败！');
	...
	public function youqingEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				operation_log(UID, 1, "友情链接编辑");
				$rs = M('Link')->save($_POST);
			} else {
				operation_log(UID, 1, "友情链接添加");
				$rs = M('Link')->add($_POST);
			}
	...
	public function youqingStatus($id = NULL, $type = NULL, $mobile = 'Link')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "友情链接状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "友情链接状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "友情链接状态设置废除");
			break;

		case 'delete':
			$data = array('status' => -1);
			operation_log(UID, 1, "友情链接状态设置删除");
			break;

		case 'del':
			operation_log(UID, 1, "友情链接删除");
			if (M($mobile)->where($where)->delete()) {
				$this->success('操作成功！');
			} else {
				$this->error('操作失败！');

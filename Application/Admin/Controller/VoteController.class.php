...
class VoteController extends AdminController
{
	...
	public function type($p=1, $r=15, $str_addtime='', $end_addtime='', $order='', $status='', $type='', $field='', $name='')
	{
        ...
        foreach ($data as $k => $vv) {
            $data[$k]['zhichi'] = M('Vote')->where(array('coinname'=>$vv['coinname'], 'type'=>1))->sum('num') + $vv['zhichi'];
            $data[$k]['fandui'] = M('Vote')->where(array('coinname'=>$vv['coinname'], 'type'=>2))->sum('num') + $vv['fandui'];
		...
		$builder->button('resume', '启 用', U('Vote/status', array('model' => 'VoteType', 'status' => 1)));
		$builder->button('forbid', '禁 用', U('Vote/status', array('model' => 'VoteType', 'status' => 0)));
	...
	public function edit($id=NULL)
	{
		if (!empty($_POST)) {
			if (check($_POST['id'], 'd')) {
                ...
                operation_log(UID, 1, "发币投票配置添加");
				$rs = M('VoteType')->add($array);
			}

			if ($rs) {
				$this->success('操作成功',U('Vote/type'));
			} else {
				$this->error('操作失败');
			}
		} else {

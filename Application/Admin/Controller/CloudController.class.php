...
class CloudController extends AdminController
{
    ...
    public function theme()
    {
            ...
            if ($file) {
                foreach ($file as $k => $v) {
                    if (($v != '.') && ($v != '..')) {
                        $fileArr[$v] = 1;
						$ds = '';
                        if (file_get_contents($dir . '/' . $v . '/conf.json')) {
                            $conf[$v] = file_get_contents($dir . '/' . $v . '/conf.json');
							$ds = json_decode($conf[$v], true);
							$conf['title'][$v] = $ds['title'];
							$conf['imttitle'][$v] = $ds['imttitle'];
							$conf['imgswitch'][$v] = $ds['imgswitch'];
							$conf['preview'][$v] = $ds['preview'];
                        } else {
                            $conf[$v] = '';
                        }
                    }
        ...
        if (!in_array('index_html', $DbFields)) {
            M()->execute('ALTER TABLE `tw_config` ADD COLUMN `index_html` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
	...
	public function jiancha_kaishi()
	{
		if (!M("menu")->where(array("url" => "Cloud/update"))->find()) {
			M()->execute("INSERT INTO `tw_menu` (`title`, `pid`, `sort`, `url`,`group`,`ico_name`) VALUES ('自动升级', '10', '1', 'Cloud/update','扩展管理','time')");
		}

		if (!M("menu")->where(array("url" => "Cloud/jiancha"))->find()) {
			M()->execute("INSERT INTO `tw_menu` (`title`, `pid`, `sort`, `url`,`group`,`ico_name`) VALUES ('安全检查', '10', '2', 'Cloud/jiancha','扩展管理','time')");
		}
	}
	...
	public function getFileList()
	{
		...
		$authorization = D("Version")->check_authorization();
		if (strtotime($authorization["shouhou"]) < time()) {

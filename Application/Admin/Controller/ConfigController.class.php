...
class ConfigController extends AdminController
{
	...
	{
		$this->data = M('Config')->where(array('id' => 1))->field('contact_myzr,contact_myzc,contact_service_qq')->find();
		$this->display();
	}
	...
	public function contactEdit()
	{
		...
		//允许修改的值
		$allow_config = array('contact_myzr', 'contact_myzc', 'contact_service_qq');
		foreach ($_POST as $key => $value) {
			if(!in_array($key, $allow_config)){
				unset($_POST[$key]);
			}
		}

		operation_log(UID, 1, "客服设置编辑");
		if (M('Config')->where(array('id' => 1))->save($_POST)) {
			$this->success('修改成功！');
		} else {
			$this->error('修改失败');
...
/*			$_POST['zc_min'] = intval($_POST['zc_min']);
	...
	public function coinStatus()
	{
		...
		switch (strtolower($method)) {
			case 'forbid':
				$data = array('status' => 0);
				operation_log(UID, 1, "币种状态设置禁止");
				break;

			case 'resume':
				$data = array('status' => 1);
				operation_log(UID, 1, "币种状态设置开启");
				break;
	...
	public function coinInfo($coin)
	{
		...
		if (strtolower($coin)=='eth'||strtolower($coin)=='etc'||$coin_config['token_type']==1){ //ETH对接,FFF
			...
			$info['coin'] = $coin;
			$info['version'] = $CoinClient->web3_clientVersion();
			...
			$coinInfo = $CoinClient->getinfo();
			$info['coin'] = $coin;
			$info['version'] = $coinInfo['version'];
			$info['blocks'] = $coinInfo['blocks'];
			$info['balance'] = round($coinInfo['balance'],8);
			$info['payfee'] = $coinInfo['paytxfee'];//wei单位显示;
			$info['num'] = M('UserCoin')->sum($coin) + M('UserCoin')->sum($coin . 'd');
			$info['totalInfo'] = $coinInfo;
		}else{
			$CoinClient = CoinClient($dj_username, $dj_password, $dj_address, $dj_port);
			if (!$CoinClient) {
				$this->error('钱包对接失败！');
	...
	public function coinUser($coin)
	{
		...
		if (strtolower($coin)=='eth' || strtolower($coin)=='etc' || $coin_config['token_type'] == 1) {//ETH对接20170920,FFF
		...
		if (strtolower($coin)=='eth' || strtolower($coin)=='etc' || $coin_config['token_type'] == 1) {//ETH对接20170920,FFF
			foreach ($arr as $k => $v) {
				...
				$list[$k]['balance'] = (hexdec($numb))/1000000000000000000;//转换成ether单位显示
				$str .= $v . '<br>';
			...
			foreach ($arr as $k => $v) {
				if ($k) {
					...
					$str = '';
					$k = $k.'';
					$addr = $CoinClient->etaddressesbylabel($v);
					$addr = json_decode($addr, true);
					//所有地址
					$json = $CoinClient->omni_getbalance($addr['address'],31);
					if($json['balance']){
						$balance = $json['balance'];
					}
					$list[$k]['balance'] = $balance;
					$list[$k]['addr'] = $addr['address']. '<br>'.$addr['purpose'];
			...
			foreach ($arr as $k => $v) {
				if ($k) {
					...
					$str = '';
					$k = $k.'';
					$list[$k]['balance'] = $CoinClient->getbalance();
					$addr = $CoinClient->etaddressesbylabel($v);
					$addr = json_decode($addr, true);
					$list[$k]['addr'] = $addr['address']. '<br>'.$addr['purpose'];
					$userid = M('User')->where(array('username' => $k))->getField('id');
	...
	public function daohangStatus($id = NULL, $type = NULL, $mobile = 'Daohang')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "前端导航状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "前端导航状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "前端导航状态设置废除");
			break;

		case 'del':
			$data = array('status' => -1);
			operation_log(UID, 1, "前端导航状态设置删除");
			break;

		case 'delete':
			operation_log(UID, 1, "前端导航删除");
	...
	public function dhfooterEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				operation_log(UID, 1, "页脚导航设置编辑");
				$rs = M('footer')->save($_POST);
			} else {
				$_POST['addtime'] = time();
				operation_log(UID, 1, "页脚导航设置添加");
	...
	public function dhfooterStatus($id = NULL, $type = NULL, $mobile = 'footer')
	{
		...
		case 'forbid':
			$data = array('status' => 0);
			operation_log(UID, 1, "页脚导航状态设置禁止");
			break;

		case 'resume':
			$data = array('status' => 1);
			operation_log(UID, 1, "页脚导航状态设置开启");
			break;

		case 'repeal':
			$data = array('status' => 2, 'endtime' => time());
			operation_log(UID, 1, "页脚导航状态设置废除");
			break;

		case 'del':
			$data = array('status' => -1);
			operation_log(UID, 1, "页脚导航状态设置删除");
			break;

		case 'delete':
			operation_log(UID, 1, "页脚导航删除");
			if (M($mobile)->where($where)->delete()) {
				$this->success('操作成功！');
			} else {
				$this->error('操作失败！');
	...
	//编辑后台导航，id：编辑的信息ID， lv：三级目录
	public function dhadminEdit($id = NULL,$thirdlv=0)
	{
		if (empty($_POST)) {
			$liste = '';
			...
			if ($id) {
				if($thirdlv){
					$this->data = null;
					$title = M('menu')->where(array('id' => trim($id)))->getField('title');
					$this->assign('third_pid', $id);
					$this->assign('third_title', $title);
				}else{
			...
			if ($_POST['id']) {
				operation_log(UID, 1, "后台导航设置编辑");
				$rs = M('menu')->save($_POST);
			} else {
				$_POST['addtime'] = time();
				operation_log(UID, 1, "后台导航设置添加");
				$rs = M('menu')->add($_POST);
			}
	...
	public function dhadminStatus($id = NULL, $type = NULL, $mobile = 'menu')
	{
		...
		switch (strtolower($type)) {
			case 'forbid':
				$data = array('hide' => 1);
				operation_log(UID, 1, "后台导航状态设置禁止");
				break;

			case 'resume':
				$data = array('hide' => 0);
				operation_log(UID, 1, "后台导航状态设置开启");
				break;

			case 'repeal':
				$data = array('hide' => 2);
				operation_log(UID, 1, "后台导航状态设置废除");
				break;
				
			case 'delete':
				$data = array('status' => -1);
				operation_log(UID, 1, "后台导航状态设置删除");
				break;

			case 'del':
				operation_log(UID, 1, "后台导航删除");
				if (M($mobile)->where($where)->delete()) {
					$this->success('操作成功！');
				} else {
					$this->error('操作失败！');
	...
	public function marketoStatus($id = NULL, $type = NULL, $mobile = 'Market')
	{
		...
		switch (strtolower($type)) {
			case 'forbid':
				$data = array('status' => 0);
				operation_log(UID, 1, "市场状态设置禁止");
				break;

			case 'resume':
				$data = array('status' => 1);
				operation_log(UID, 1, "市场状态设置开启");
				break;

			case 'repeal':
				$data = array('status' => 2, 'endtime' => time());
				operation_log(UID, 1, "市场状态设置废除");
				break;

			case 'delete':
				$data = array('status' => -1);
				operation_log(UID, 1, "市场状态设置删除");
				break;

			case 'del':
				operation_log(UID, 1, "市场删除");
				if (M($mobile)->where($where)->delete()) {
					$this->success('操作成功！');
				} else {
					$this->error('操作失败！');
	...
	public function tradeclear($type=NULL,$id=NULL)
	{
		...
		if ($type == 1) {
			operation_log(UID, 1, "币币交易市场机器人数据清理");
		...
		if ($type == 2) {
			...
			operation_log(UID, 1, "币币交易市场无效机器人数据清理");

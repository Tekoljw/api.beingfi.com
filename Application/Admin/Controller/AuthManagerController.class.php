...
class AuthManagerController extends AdminController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","updateRules","createGroup","editGroup","access","writeGroup","changeStatus","user","category","tree","group","addToGroup","removeFromGroup","addToCategory","addToModel");
	...
	public function writeGroup()
	{
		...
		if ($data) {
			if (empty($data['id'])) {
				operation_log(UID, 1, "用户组添加");
				$r = $AuthGroup->add();
			}else {
				operation_log(UID, 1, "用户组编辑");
				$r = $AuthGroup->save();
			}
	...
	public function changeStatus($method = NULL)
	{
		...
		case 'forbidgroup':
			operation_log(UID, 1, "用户组状态设置禁止");
			$this->forbid('AuthGroup');
			$this->success('操作成功', '/');
			break;

		case 'resumegroup':
			operation_log(UID, 1, "用户组状态设置开启");
			$this->resume('AuthGroup');
			$this->success('操作成功', '/');
			break;

		case 'deletegroup':
			operation_log(UID, 1, "用户组状态设置删除");
